...
{
    private:
// millimeter (mm) ~ the values of each atribute below are equal or close to their convetion factors
...
// Convert each character of the string to touppercase
     ...
     // asking the user, if the user want to use the sofware agian
        NewLine();
            HeaderFooterDesign();
                Decor1Tab();
                    std::cout << "Enter [ YES / Y ] if you wish to use this software again, otherwise enter any keys" << std::endl;
                        Decor1Tab();
                            UseriputLine();
                                std::cin >> User_YesNo;
 ...
 // display at the end
    Decor1Tab();
        NewLine();
            std::cout << "Weight-Lenght Master & BMI-Wizard" << std::endl;
                NewLine();
    return 0;
}


/* Weight-Lenght_Master_BMI-Wizard.cxx / Weight Lenght Master BMI Wizard */